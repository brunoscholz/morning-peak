...
class DatabaseUtil
{
    ...
    public static function truncateTables($name = '')
    {
        ...
        AppAdaptor::db()->createCommand('SET FOREIGN_KEY_CHECKS = 0;')->execute();
        foreach ($tableNames as $tableName)
        {
            self::truncateTable($tableName);
        }
        AppAdaptor::db()->createCommand('SET FOREIGN_KEY_CHECKS = 1;')->execute();
    }
    ...
    {
        $results = AppAdaptor::db()->getSchema()->getTableNames();
        if (!empty($results))
        {
            AppAdaptor::db()->createCommand('SET FOREIGN_KEY_CHECKS = 0;')->execute();
            foreach ($results as $row)
            {
                AppAdaptor::db()->createCommand()->dropTable($row)->execute();
            }
            AppAdaptor::db()->getSchema()->refresh();
            AppAdaptor::db()->createCommand('SET FOREIGN_KEY_CHECKS = 1;')->execute();
        }
    }
    ...
    {
        return $query->prepare(AppAdaptor::app()->db->queryBuilder)->createCommand()->rawSql;
    }
}