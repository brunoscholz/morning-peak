...
class FlashUtil
{
    ...
    public static function getMessage($key, $params = array())
    {
        ...
        {
            return AppAdaptor::t('application', '(not set)');
        }
    }
    ...
    private static function resolveMessage($key, $class)
    {
        ...
        if (AppAdaptor::app()->session->hasFlash($key))
        {
            if (is_string($class))
            {
                return UiHtml::tag('div', AppAdaptor::app()->session->getFlash($key), ['class' => $class]);
            }
            else
            {
                throw new \yii\web\HttpException(500, AppAdaptor::t('application', 'Invalid css class passed for the flash message with key as {key}',
                                        array('{key}' => $key)));
            }
        }
        return null;
    ...
    {
        AppAdaptor::app()->getSession()->setFlash($key, $flashMessage);
    }
}